  ...
  for (int np = 0; np<N; np++) {
    ...
    if (l == 0) {                                               // For initial level
      for (int n = 0; n < nf; n++) {
        ...
        Xf[1] *= 1 + r*hf + rho*sigma2*dWf[0]                   // Euler and Maruyama terms
          + crho*sigma2*dWf[1];
      ...
      for (int n = 0; n<nc; n++) { 
        ...
        Xf[1] *= 1 + r*hf + rho*sigma2*dWf[0]                   // Euler and Maruyama terms, Eq 2, fine discretization, refinement step 1/2
          + crho*sigma2*dWf[1];

        Xf[0] *= 1 + r*hf + sigma1*dWf[2];                      // Euler and Maruyama terms, Eq 1, fine discretization, refinement step 2/2
        Xf[1] *= 1 + r*hf + rho*sigma2*dWf[2]                   // Euler and Maruyama terms, Eq 2, fine discretization, refinement step 2/2
          + crho*sigma2*dWf[3];

        Xc[0] *= 1 + r*hc + sigma1*dWc[0];                      // Euler and Maruyama terms, Eq 1, coarse discretization
        Xc[1] *= 1 + r*hc + rho*sigma2*dWc[0]                   // Euler and Maruyama terms, Eq 2, coarse discretization
          + crho*sigma2*dWc[1];
