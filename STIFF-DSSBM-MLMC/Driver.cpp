/*/////////////////////////////////////////////////////////////
  ...
  THANKS:
    ...
    (originally named mcqmc06_l), which is now in a new file
  ...
  for (int np = 0; np<N; np++) {
    ...
    //if (l == __FIRST_LEVEL__) {
      for (int n = 0; n < nf; n++) {
        ...
        Xf[0] /= 1 - c1*hf;                                     // DRSSE - Implicit Euler term
        Xf[1] /= 1 - c2*hf;                                     // DRSSE - Implicit Euler term
        Xf[1] += 4.0*Xf[0]*dWf[0] + 0.5*dWf[1];                 // Maruyama term
      ...
      for (int n = 0; n<nc; n++) { 
        ...
        Xf[0] /= 1 - c1*hf;                                     // DSSBM - Implicit Euler term
        Xf[1] /= 1 - c2*hf;                                     // DSSBM - Implicit Euler term
        Xf[1] += 4.0*Xf[0]*dWf[0] + 0.5*dWf[1];                 // Maruyama term

        Xf[0] /= 1 - c1*hf;                                     // DSSBM - Implicit Euler term
        Xf[1] /= 1 - c2*hf;                                     // DSSBM - Implicit Euler term
        Xf[1] += 4.0*Xf[0]*dWf[2] + 0.5*dWf[3];                 // Maruyama term

        Xc[0] /= 1 - c1*hc;                                     // DSSBM - Implicit Euler term
        Xc[1] /= 1 - c2*hc;                                     // DSSBM - Implicit Euler term
        Xc[1] += 4.0*Xc[0]*dWc[0] + 0.5*dWc[1];                 // Maruyama term
