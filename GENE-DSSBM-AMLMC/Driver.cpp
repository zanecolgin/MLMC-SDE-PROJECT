/*/////////////////////////////////////////////////////////////
  ...
  using Drift Split-Step Backwards Milstein method and 
    antithetic multilevel Monte Carlo
...
void solve_timestep(double* Y1p, double* Y2p, double* Y3p, double h,
  ...
  static const double sq2c3 = sqrt(2.0*c3);                            // sqrt(2*c3)
  ...
  barY1 = (Y1 + c1G0*h) / (1.0 + c4*h);
  a = c3*h;
  b = 1 - c3*h + c5*h;
  c = -Y2 - c2*h*barY1;
  barY2 = (-b + sqrt(b*b - 4.0*a*c)) / 2.0 / a;
  barY3 = Y3 + c3_2*barY2*(barY2 - 1.0)*h;
  ...
  Y2Y21 = barY2*(barY2 - 1.0);
  Y205 = barY2 - 0.5;

  sqY21 = sqrt(barY2 - 1.0);
  sqY2Y21 = sqrt(Y2Y21);
